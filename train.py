...
def training(dataset, opt, pipe, testing_iterations, saving_iterations, checkpoint_iterations, checkpoint, debug_from, pseudo_view):
    ...
    # MODIFIED CODE - creating test cameras
    if dataset.test_path != "":
        train_images = [cm.image_name for cm in  scene.train_cameras[1.0]]
        all_scene_infos = sceneLoadTypeCallbacks["Colmap"](path=dataset.test_path,
                                                           images='images',
                                                           depths="",
                                                           eval=False,train_test_exp=False, llffhold=8)
        for caminfo in all_scene_infos.train_cameras: 
            if caminfo.image_name not in train_images:
                all_scene_infos.test_cameras.append(caminfo)
        for resolution_scale in scene.train_cameras.keys():
            print("Re-Loading Test Cameras")
            scene.test_cameras[resolution_scale] = cameraList_from_camInfos(all_scene_infos.test_cameras,
                                                                            resolution_scale, dataset,
                                                                            all_scene_infos.is_nerf_synthetic, False)
    ...
    for iteration in range(first_iter, opt.iterations + 1):
        ...
        with torch.no_grad():
            ...
            # Log and save
            training_report(dataset.model_path, tb_writer, iteration, Ll1, loss, l1_loss, iter_start.elapsed_time(iter_end), testing_iterations, scene, render, (pipe, background, 1., SPARSE_ADAM_AVAILABLE, None, dataset.train_test_exp), dataset.train_test_exp)
...
def training_report(model_path, tb_writer, iteration, Ll1, loss, l1_loss, elapsed, testing_iterations, scene : Scene, renderFunc, renderArgs, train_test_exp):
    if tb_writer:
        ...
        for config in validation_configs:
            if config['cameras'] and len(config['cameras']) > 0:
                ...
                for idx, viewpoint in enumerate(config['cameras']):
                    ...
                    # save test images
                    savepath = Path.cwd()/model_path/f'results_{iteration}'
                    Path.mkdir(savepath, exist_ok=True, parents=True)
                    img_np = (image.cpu().permute(1,2,0).numpy() * 255).astype('uint8')
                    Image.fromarray(img_np).save(savepath/f'{viewpoint.image_name}')
                ...
                with open(f'{model_path}/log.jsonl', 'a') as f:
                    report = {'epoch': iteration, "type": config['name'], 'l1_loss': l1_test.item(), 'psnr': psnr_test.item(), 'ssim': ssim_test.flatten()[0].item(), 'lpips': lpips_test.flatten()[0].item()}
                    json.dump(report, f)
                    f.write('\n')
